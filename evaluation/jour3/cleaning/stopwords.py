...
def clean_word_list(text):
    return [word for word in text.split() if word not in stoplist]
...
exo_clean_word_list = ExerciseFunction(
    clean_word_list,
    arguments,
)
...
exo_remove_legal_stopwords = ExerciseFunction(
    remove_legal_stopwords,
    arguments,
)
...
text=[
    Args("""Junkfood - Food that do no good to our body. And there's no need of them in our body but still we willingly eat them because they are great in taste and easy to cook or ready to eat. Junk foods have no or very less nutritional value and irrespective of the way they are marketed, they are not healthy to consume.The only reason of their gaining popularity and increased trend of consumption is 
